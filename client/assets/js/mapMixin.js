...
var mapMixin = {
  ...
  methods: {
    setUpMap(obj)
    {
      ...
      if (targetObject.lat != '') {
        zoom = 12;
        location = {lat: targetObject.lat, lng: targetObject.lng};
      }
      else {
        zoom = 4;
    ...
    getPosition(objectPointer, resolve, reject)
    {
      ...
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        if (objectPointer != null) {
          ...
          this.$set(targetObject, 'lat', position.coords.latitude);
          this.$set(targetObject, 'lng', position.coords.longitude);
        }
    ...
    geocodeLatLng(obj, precision) {
      ...
      geocoder.geocode({
        'location': {
          lat: parseFloat(targetObject.lat),
          lng: parseFloat(targetObject.lng)
        }
    ...
    addMarkerToMap(obj, targetArray, callBack) {
      ...
      var icon = require('./../../../static/marker.png');
      var iconSelected = require('./../../../static/marker1.png');
      ...
      function addMarker(targetObject, callBack) {
        ...
        var currentLatLng = {lat: parseFloat(targetObject.lat), lng: parseFloat(targetObject.lng)};
    ...
    fillPlaceData(obj, place, precision) {
      ...
      this.$set(targetObject, 'lat', place.geometry.location.lat());
      this.$set(targetObject, 'lng', place.geometry.location.lng());
    },
